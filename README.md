...
- Individually tuned vs. HyperParamEnsemble (single-therapy): Hyperparameter-voting ensembles did not consistently outperform the best single tuned model; they produced small, metric-dependent changes—often improving macro-F1 (more balanced class-wise performance) while offering little or inconsistent accuracy gains. Overall, the ensemble mainly acted as a stability/robustness booster rather than a reliable way to raise top-line accuracy.
